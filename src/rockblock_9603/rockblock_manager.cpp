...
static constexpr uint32_t BOOT_DELAY_MS        = 5000;  // FIX 2: sensor warmup
...
// ======================================================
// FIX 3: wake_and_verify — only wait WAKE_SETTLE_MS if
//         modem was actually sleeping
// ======================================================
...
        sleep_ms(WAKE_SETTLE_MS);  // only wait if we actually woke it
    }
    // else already awake — no settle wait needed
...
// ======================================================
// transmit_payload — with full retry logic
// ======================================================
...
        // ── Write message — retry up to 3 times ──────────
        bool write_ok = false;
...
        {
            // FIX 4: MT check only on successful session
...
            _state          = ManagerState::SLEEPING;
            _last_tx_ms     = monotonic_ms();  // FIX 1: cooldown on permanent fail too
            return false;
        }
...
    // FIX 1: All retries exhausted — set _last_tx_ms so cooldown applies
...
// ======================================================
// force_transmit — bypasses buffer, sends one record now
// ======================================================
...
// ======================================================
// Task — buffers records, transmits when 3 accumulated
//         or interval elapsed with at least 1 record
// ======================================================
...
    // FIX 2: Boot delay — let sensors stabilise
...
    // Transmit when buffer full, OR interval elapsed and we have data
    bool should_tx = buffer_full || (interval_elapsed && _buffer_count > 0);

    if (!should_tx)
        return true;

    // ── Pack all buffered records into payload ────────────
...
    // Always clear buffer after attempt (success or fail)
    _buffer_count = 0;
